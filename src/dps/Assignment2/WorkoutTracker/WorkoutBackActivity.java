...
public class WorkoutBackActivity extends Activity {
	private static final String CURRENT_CATEGORY = "back";
	private int mYear;
	private int mMonth;
	private int mDay;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.workout_back);
        
        weightField = (EditText)findViewById(R.id.back_edit_weight);
        repsField = (EditText)findViewById(R.id.back_edit_reps);
        exerciseDisplay = (LinearLayout)findViewById(R.id.back_exercise_layout);
        ...
        addBtn = (Button)findViewById(R.id.back_addBtn);
        addBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if (repsField.getText().toString().equals("") || weightField.getText().toString().equals("") ) {
					Toast.makeText(WorkoutBackActivity.this, "Fill out all required fields first.", Toast.LENGTH_LONG).show();
				}
				else {
					...
					Toast.makeText(WorkoutBackActivity.this, "Added Exercise", Toast.LENGTH_LONG).show();
					updateExerciseDisplay();
				}
			}
        });
        ...
        // Populate spinner
        exerciseSpinner = (Spinner) findViewById(R.id.spinner_back);
        ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(
                this, R.array.exercises_array_back, android.R.layout.simple_spinner_item);
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        exerciseSpinner.setAdapter(adapter);
        
        // Update List of Back Exercises performed at the bottom, if applicable 
        updateExerciseDisplay();
    ...
    private void updateExerciseDisplay() {
    	ArrayList<Exercise> backExercises = WorkoutStartTabWidget.currentWorkout.getExercisesByCategory(CURRENT_CATEGORY);
    	exerciseDisplay.removeAllViews();
    	for (int i = 0; i < backExercises.size(); i++) {
    	  	TextView newEntry = new TextView(this);
    		String entryText = backExercises.get(i).getName() + ":  " + backExercises.get(i).getWeight() + " x " + backExercises.get(i).getReps();

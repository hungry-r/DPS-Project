...
public class WorkoutCardioActivity extends Activity {
	private static final String CURRENT_CATEGORY = "cardio";
	private int mYear;
	private int mMonth;
	private int mDay;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.workout_cardio);
        
        weightField = (EditText)findViewById(R.id.cardio_edit_weight);
        repsField = (EditText)findViewById(R.id.cardio_edit_reps);
        exerciseDisplay = (LinearLayout)findViewById(R.id.cardio_exercise_layout);
        ...
        addBtn = (Button)findViewById(R.id.cardio_addBtn);
        addBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if (repsField.getText().toString().equals("") || weightField.getText().toString().equals("") ) {
					Toast.makeText(WorkoutCardioActivity.this, "Fill out all required fields first.", Toast.LENGTH_LONG).show();
				}
				else {
					...
					WorkoutDbHelper handler = new WorkoutDbHelper(WorkoutCardioActivity.this);
					handler.addWorkoutExercise(newExercise);
					handler.close();
					WorkoutStartTabWidget.currentWorkout.addExercise(newExercise);
					Toast.makeText(WorkoutCardioActivity.this, "Added Exercise", Toast.LENGTH_LONG).show();
					updateExerciseDisplay();
				}
			}
        });
        ...
        // Populate spinner
        exerciseSpinner = (Spinner) findViewById(R.id.spinner_cardio);
        ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(
                this, R.array.exercises_array_cardio, android.R.layout.simple_spinner_item);
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        exerciseSpinner.setAdapter(adapter);
        
        // Update List of Cardio Exercises performed at the bottom, if applicable 
        updateExerciseDisplay();
    ...
    private void updateExerciseDisplay() {
    	ArrayList<Exercise> cardioExercises = WorkoutStartTabWidget.currentWorkout.getExercisesByCategory(CURRENT_CATEGORY);
    	exerciseDisplay.removeAllViews();
    	for (int i = 0; i < cardioExercises.size(); i++) {
    	  	TextView newEntry = new TextView(this);
    		String entryText = cardioExercises.get(i).getName() + ":  " + cardioExercises.get(i).getWeight() + " x " + cardioExercises.get(i).getReps();

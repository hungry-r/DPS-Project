...
public class WorkoutArmsActivity extends Activity {
	private static final String CURRENT_CATEGORY = "arms";
	private int mYear;
	private int mMonth;
	private int mDay;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.workout_arms);
        
        weightField = (EditText)findViewById(R.id.arms_edit_weight);
        repsField = (EditText)findViewById(R.id.arms_edit_reps);
        exerciseDisplay = (LinearLayout)findViewById(R.id.arms_exercise_layout);
        ...
        addBtn = (Button)findViewById(R.id.arms_addBtn);
        addBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if (repsField.getText().toString().equals("") || weightField.getText().toString().equals("") ) {
					Toast.makeText(WorkoutArmsActivity.this, "Fill out all required fields first.", Toast.LENGTH_LONG).show();
				}
				else {
					...
					WorkoutDbHelper handler = new WorkoutDbHelper(WorkoutArmsActivity.this);
					handler.addWorkoutExercise(newExercise);
					handler.close();
					WorkoutStartTabWidget.currentWorkout.addExercise(newExercise);
					Toast.makeText(WorkoutArmsActivity.this, "Added Exercise", Toast.LENGTH_LONG).show();
					updateExerciseDisplay();
				}
			}
        });
        ...
        // Populate spinner
        exerciseSpinner = (Spinner) findViewById(R.id.spinner_arms);
        ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(
                this, R.array.exercises_array_arms, android.R.layout.simple_spinner_item);
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        exerciseSpinner.setAdapter(adapter);
        
        // Update List of Arms Exercises performed at the bottom, if applicable 
        updateExerciseDisplay();
    ...
    private void updateExerciseDisplay() {
    	ArrayList<Exercise> armsExercises = WorkoutStartTabWidget.currentWorkout.getExercisesByCategory(CURRENT_CATEGORY);
    	exerciseDisplay.removeAllViews();
    	for (int i = 0; i < armsExercises.size(); i++) {
    	  	TextView newEntry = new TextView(this);
    		String entryText = armsExercises.get(i).getName() + ":  " + armsExercises.get(i).getWeight() + " x " + armsExercises.get(i).getReps();
